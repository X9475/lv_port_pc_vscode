...
static lv_page_info_pt current_page = NULL;
...
static void page_switch_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    {
        lv_page_transition_anim_create(&switch_page->anim_transt, anim_completed_cb, 
                                        switch_page, anim_completed_cb, switch_page);
    }
    else
    {
    ...
    if (g_device_state == LV_PAGE_STAGE_RUNNING)
    {
        ...
        if (new_page != lv_page_menu_info_get() && new_page != lv_page_menu_setting_info_get())
        {
            ...
            if (!page_add_gesture_event_filter(new_page))
            {
                ...
                lv_obj_set_size(top_gesture_area, lv_pct(100), 30);
                ...
                lv_obj_set_size(bottom_gesture_area, lv_pct(100), 30);
...
static void page_gesture_event_hander(lv_event_t *e)
{
    ...
    if (dir == 0) return;
    printf("===> gesture dir: %d, %d\n", dir, lv_page_type_get());

    switch (dir)
    {
        case LV_DIR_BOTTOM:
            if (lv_page_type_get() == TYPE_FUNC)
            {
                lv_stack_push(current_page);
                ...
                switch_menu_setting->old_page = current_page;
            ...
            else if (lv_page_type_get() == TYPE_MENU)
            {
                ...
                lv_stack_push(lv_page_menu_info_get());
                ...
                switch_functional->new_page = last_page == NULL? lv_page_menu_setting_info_get() : last_page;
                switch_functional->old_page = lv_page_menu_info_get();

                //动画参数设置
                switch_functional->anim_transt.anim_support = true;
                switch_functional->anim_transt.new_type = LV_ANIM_BOX_NONE;
        ...
        case LV_DIR_TOP:
            if (lv_page_type_get() == TYPE_FUNC)
            {
                ...
                switch_menu->old_page = current_page;
            ...
            else if (lv_page_type_get() == TYPE_MENUSET)
            {
                lv_page_info_pt last_page = lv_stack_pop();
                lv_stack_push(lv_page_menu_setting_info_get());

                //回到功能界面
                switch_functional = (lv_switch_page_pt)lv_malloc(sizeof(lv_switch_page_t));
                LV_ASSERT_MALLOC(switch_functional);
                lv_memset(switch_functional, 0, sizeof(lv_switch_page_t));
                switch_functional->new_page = last_page == NULL? lv_page_menu_info_get() : last_page;
