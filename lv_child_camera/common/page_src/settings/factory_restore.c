...
lv_subject_t factory_restore_subject;
...
static void lv_page_subject_init()
{
    lv_subject_init_int(&factory_restore_subject, PAGE_SWITCH_NONE);
    lv_subject_add_observer(&factory_restore_subject, lv_switch_observer_cb, NULL);
    return;
}

static void lv_page_subject_deinit()
{
    lv_subject_deinit(&factory_restore_subject);
}
...
static void lv_cancel_and_confirm_click_event(lv_event_t *e)
{
    ...
    if (btn == confirm)
    {
        lv_subject_set_int(&factory_restore_subject, PAGE_SWITCH_CONFIRM);
    }
    else if (btn == cancel)
    {
...
static void lv_switch_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    switch_page->old_page = &factory_restore_page_info;
