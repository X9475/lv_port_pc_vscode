...
static void btn_confirm_click_event(lv_event_t *e);
...
static void lv_page_load(lv_obj_t *cont)
{
    ...
    lv_obj_t *confirm = lv_btn_create(cont);
    ...
    lv_obj_align(confirm, LV_ALIGN_BOTTOM_MID, 0, -12);

    lv_obj_t *icon = lv_img_create(confirm);
    lv_obj_set_size(icon, 50, 50);
    lv_img_set_src(icon, "../lv_port_pc_vscode/assert/icon/common_icon_ok_button.png");
    lv_obj_align_to(icon, confirm, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_image_recolor_opa(icon, LV_OPA_COVER, 0);
    lv_obj_set_style_image_recolor(icon, lv_color_hex(0x0A0B0D), 0);
    lv_obj_add_event_cb(confirm, btn_confirm_click_event, LV_EVENT_CLICKED, confirm);
...
static void btn_confirm_click_event(lv_event_t *e)
{
    if (indvid_key_select == 0)
    {
        printf("拍摄\n");
    }
    else if (indvid_key_select == 1)
    {
        printf("AI问答\n");
    }
}
