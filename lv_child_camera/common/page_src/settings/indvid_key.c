...
static void lv_page_style_init()
{
    ...
    grad0.stops[0].opa = LV_OPA_COVER;
    grad0.stops[1].color = lv_color_hex(0x000000);
    grad0.stops[1].opa = LV_OPA_TRANSP;
    grad0.stops[0].frac = 0;
    grad0.stops[1].frac = 255;
    lv_style_init(&up_style);
    lv_style_copy(&up_style, &screen_style);
    lv_style_set_bg_grad(&up_style, &grad0);
    ...
    lv_style_set_bg_grad(&bottom_style, &grad1);
}
...
static void lv_page_load(lv_obj_t *cont)
{
    ...
    lv_obj_set_size(contain, lv_pct(100), 297);
    ...
    lv_obj_align(contain, LV_ALIGN_TOP_MID, 0, 113);

    lv_obj_t *product_img = lv_img_create(contain);
    lv_obj_set_size(product_img, 264, 157);
    lv_img_set_src(product_img, "../lv_port_pc_vscode/assert/icon/key_yellow.png");
    lv_obj_align(product_img, LV_ALIGN_TOP_MID, 0, 0);

    //上部渐变
    lv_obj_t *up_mask = lv_obj_create(contain);
    lv_obj_set_size(up_mask, 474, 37);
    lv_obj_add_style(up_mask, &up_style, 0);
    lv_obj_align(up_mask, LV_ALIGN_TOP_MID, 0, 0);
    //下部渐变
    lv_obj_t *bottom_mask = lv_obj_create(contain);
    lv_obj_set_size(bottom_mask, 474, 72);
    lv_obj_add_style(bottom_mask, &bottom_style, 0);
    lv_obj_align(bottom_mask, LV_ALIGN_TOP_MID, 0, 85);

    lv_obj_t *descrip = lv_label_create(contain);
    lv_label_set_text(descrip, "你想通过此按键");
    ...
    lv_obj_align(descrip, LV_ALIGN_TOP_LEFT, 40, 200);
    ...
    lv_obj_align(cont_col, LV_ALIGN_TOP_MID, 0, 231);
