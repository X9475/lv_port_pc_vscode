...
static lv_obj_t *ai_icon;
static lv_obj_t *audio_effect;
...
static void lv_page_style_init()
{
    ...
    lv_style_copy(&btn_style, &screen_style);
...
static void lv_page_load(lv_obj_t *cont)
{
    ...
    //AI
    ai_icon = lv_obj_create(cont);
    lv_obj_set_size(ai_icon, 100, 100);
    lv_obj_add_style(ai_icon, &btn_style, 0);
    lv_obj_set_style_radius(ai_icon, LV_RADIUS_CIRCLE, 0);
    lv_obj_align(ai_icon, LV_ALIGN_TOP_LEFT, 145, 35);
    lv_obj_add_flag(ai_icon, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(ai_icon, lv_event_handler_code, LV_EVENT_CLICKED, NULL);

    lv_obj_t *ai_img = lv_img_create(ai_icon);
    lv_obj_align(ai_img, LV_ALIGN_CENTER, 0, 0);
    lv_img_set_src(ai_img, "../lv_port_pc_vscode/assert/icon/photograph_icon_ai.png");
    // lv_img_set_zoom(ai_img, 128);

    //音频效果
    audio_effect = lv_obj_create(cont);
    lv_obj_set_size(audio_effect, 210, 100);
    lv_obj_add_style(audio_effect, &btn_style, 0);
    lv_obj_set_style_radius(audio_effect, 50, 0);
    lv_obj_align(audio_effect, LV_ALIGN_LEFT_MID, 35, 0);
    lv_obj_add_flag(audio_effect, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(audio_effect, lv_event_handler_code, LV_EVENT_CLICKED, NULL);

    lv_obj_t *video_dura_txt = lv_label_create(audio_effect);
    lv_label_set_text(video_dura_txt, "音频效果");
    lv_obj_add_style(video_dura_txt, &label_style, 0);
    lv_obj_align(video_dura_txt, LV_ALIGN_CENTER, 0, 0);

    //锁屏
    screensaver = lv_obj_create(cont);
    lv_obj_set_size(screensaver, 210, 100);
    lv_obj_add_style(screensaver, &btn_style, 0);
    lv_obj_set_style_radius(screensaver, 50, 0);
    lv_obj_align(screensaver, LV_ALIGN_BOTTOM_LEFT, 35, -35);
    ...
    // lv_img_set_src(bright_icon, "../lv_port_pc_vscode/assert/icon/volume_icon.png");
    // lv_img_set_zoom(bright_icon, 200);
    // lv_obj_align(bright_icon, LV_ALIGN_BOTTOM_MID, 0, -20);
    ...
    lv_img_set_src(volume_icon, "../lv_port_pc_vscode/assert/icon/volume_icon.png");
    lv_img_set_zoom(volume_icon, 200);
...
static void lv_event_handler_code(lv_event_cb_t *e)
{
    ...
    static bool ai_icon_flag = false;
    ...
    if (LV_EVENT_CLICKED == code)
    {
        ...
        {//AI
            if (!ai_icon_flag) {
                lv_obj_set_style_bg_color(ai_icon, lv_color_hex(0xCE94F8), 0);
                ai_icon_flag = true;
                //设置AI语音助手打开的标志，需要发送主题事件到AI界面回调
            }
            else
            {
                lv_obj_set_style_bg_color(ai_icon, lv_color_hex(0x000000), 0);
                ai_icon_flag = false;
            }
        }
        else if (obj == audio_effect)
        {//音频效果
