...
#define INDVID_SETTING     9

lv_subject_t indvid_setting_subject;
...
static void indvid_setting_iterm_create(lv_obj_t *cont, const char *name, int32_t index);
static void setting_iterm_click_event_cb(lv_event_cb_t *e);

static const char *setting_list[INDVID_SETTING] = {
    "设备亮度", "设备音量", "熄屏时间", "屏保样式", "自定义按键", "时间展示形式", "语音控制开关",
    "滚动振动开关", "远程预览开关"};
...
{
    PAGE_SWITCH_NONE,
    PAGE_SWITCH_BRIGHT,         //设备亮度
    PAGE_SWITCH_VOLUME,         //设备音量
    PAGE_SWITCH_HOLD_TIME,      //熄屏时间
    PAGE_SWITCH_SAVER,          //屏保样式
    PAGE_SWITCH_CUSTOM_KEY,     //自定义按键
    PAGE_SWITCH_TIME_FORMAT,    //时间展示形式
    PAGE_SWITCH_VOICE_CONTROL,  //语音控制开关
    PAGE_SWITCH_ROLL_VIBRAT,    //滚动振动开关
    PAGE_SWITCH_REMATE_PREVIEW, //远程预览开关
    PAGE_SWITCH_BACK
};

static lv_page_info_t indvid_setting_page_info = {
    .page_id = PAGE_FUNCTIONAL_INDVID_SETTINGS,
    .page = NULL,
    .reserved = NULL,
...
{
    return &indvid_setting_page_info;
}
...
static void lv_page_construct(void *this)
{
    ...
    //绘制当前页面
    lv_page_load(screen);
    indvid_setting_page_info.page = screen;
    return;
}
...
static void lv_page_subject_init()
{
    lv_subject_init_int(&indvid_setting_subject, PAGE_SWITCH_NONE);
    lv_subject_add_observer(&indvid_setting_subject, lv_switch_observer_cb, NULL);
    return;
}

static void lv_page_subject_deinit()
{
    lv_subject_deinit(&indvid_setting_subject);
}
...
static void lv_page_load(lv_obj_t *cont)
{
    ...
    lv_label_set_text(header, "个性化设置");
    ...
    for (uint8_t i = 0; i < INDVID_SETTING; i++)
    {
        indvid_setting_iterm_create(cont_col, setting_list[i], i);
    }

    int new_index = (last_tabindex + 1) > (INDVID_SETTING - 1)? (INDVID_SETTING - 1):(last_tabindex + 1);
    ...
    while(1)
    {
        ...
        if(fgets(buf, sizeof(buf), stdin))
        {
            ...
            // if (current_page->page_id == PAGE_FUNCTIONAL_MENU_SETTING)
            {
                ...
                {
                    //创建透明屏幕禁止屏幕响应
                    g_rotate_ctl_s.mask = lv_obj_create(indvid_setting_page_info.page);
                    ...
                    lv_obj_clear_flag(indvid_setting_page_info.page, LV_OBJ_FLAG_GESTURE_BUBBLE);
                    ...
                    //获取当前中间项目索引
                    lv_obj_t *cont_col = lv_obj_get_child(indvid_setting_page_info.page, 2);
...
static void async_rotate_cb(void *cmd)
{
    ...
    //边界检查
    if (current_index >= (INDVID_SETTING - 1))
    {
        current_index = INDVID_SETTING - 1;
    } 
...
static void delete_mask_page(lv_timer_t *timer)
{
    ...
    lv_obj_add_flag(indvid_setting_page_info.page, LV_OBJ_FLAG_GESTURE_BUBBLE);
...
static void setting_iterm_click_event_cb(lv_event_cb_t *e)
{
    ...
    if (LV_EVENT_CLICKED == code)
    {
        ...
        {
            last_tabindex = 0;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_BRIGHT);
        }
        else if (index == 1)
        {
            last_tabindex = 1;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_VOLUME);
        }
        else if (index == 2)
        {
            last_tabindex = 2;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_HOLD_TIME);
        }
        else if (index == 3)
        {
            last_tabindex = 3;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_SAVER);
        }
        else if (index == 4)
        {
            last_tabindex = 4;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_CUSTOM_KEY);
        }
        else if (index == 5)
        {
            last_tabindex = 5;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_TIME_FORMAT);
        }
        else if (index == 6)
        {
            last_tabindex = 6;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_VOICE_CONTROL);
        }
        else if (index == 7)
        {
            last_tabindex = 7;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_ROLL_VIBRAT);
        }
        else if (index == 8)
        {
            last_tabindex = 8;
            lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_REMATE_PREVIEW);
        }
    }
}
...
static void page_back_event_cb(lv_event_t *e)
{
    lv_subject_set_int(&indvid_setting_subject, PAGE_SWITCH_BACK);
}
...
static void lv_switch_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    switch_page->old_page = &indvid_setting_page_info;

    switch (page_event)
    {
        case PAGE_SWITCH_BRIGHT:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_device_bright_info_get();
            break;
        case PAGE_SWITCH_VOLUME:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_device_volume_info_get();
            break;
        case PAGE_SWITCH_HOLD_TIME:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_hold_time_info_get();
            break;
        case PAGE_SWITCH_SAVER:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_screensaver_style_info_get();
            break;
        case PAGE_SWITCH_CUSTOM_KEY:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_indvid_key_info_get();
            break;
        case PAGE_SWITCH_TIME_FORMAT:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_time_display_info_get();
            break;
        case PAGE_SWITCH_VOICE_CONTROL:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_voice_control_info_get();
            break;
        case PAGE_SWITCH_ROLL_VIBRAT:
            lv_stack_push(&indvid_setting_page_info);
            switch_page->new_page = lv_page_roller_vibrat_info_get();
            break;
        case PAGE_SWITCH_REMATE_PREVIEW:
            lv_stack_push(&indvid_setting_page_info);
