...
#define EQUIP_SETTING     4

lv_subject_t equip_setting_subject;
...
static void equip_setting_iterm_create(lv_obj_t *cont, const char *name, int32_t index);
static void setting_iterm_click_event_cb(lv_event_cb_t *e);

static const char *setting_list[EQUIP_SETTING] = {"设备二维码", "关于相机", "恢复出厂设置", "认证标志"};

//待跳转的页面种类
static enum PAGE_EVENT_ENUM
{
    PAGE_SWITCH_NONE,
    PAGE_SWITCH_QRCODE,  //设备二维码
    PAGE_SWITCH_ABOUT,   //关于相机
    PAGE_SWITCH_RESET,   //恢复出厂设置
    PAGE_SWITCH_CERK,    //认证标志
    PAGE_SWITCH_BACK
};

static lv_page_info_t equip_setting_page_info = {
    .page_id = PAGE_FUNCTIONAL_EQUIP_SETTINGS,
    .page = NULL,
    .reserved = NULL,
...
{
    return &equip_setting_page_info;
}
...
static void lv_page_construct(void *this)
{
    ...
    //绘制当前页面
    lv_page_load(screen);
    equip_setting_page_info.page = screen;
    return;
}
...
static void lv_page_subject_init()
{
    lv_subject_init_int(&equip_setting_subject, PAGE_SWITCH_NONE);
    lv_subject_add_observer(&equip_setting_subject, lv_switch_observer_cb, NULL);
    return;
}

static void lv_page_subject_deinit()
{
    lv_subject_deinit(&equip_setting_subject);
}
...
static void lv_page_load(lv_obj_t *cont)
{
    ...
    for (uint8_t i = 0; i < EQUIP_SETTING; i++)
    {
        equip_setting_iterm_create(cont_col, setting_list[i], i);
    }

    int new_index = (last_tabindex + 1) > (EQUIP_SETTING - 1)? (EQUIP_SETTING - 1):(last_tabindex + 1);
    ...
    while(1)
    {
        ...
        if(fgets(buf, sizeof(buf), stdin))
        {
            ...
            // if (current_page->page_id == PAGE_FUNCTIONAL_MENU_SETTING)
            {
                ...
                {
                    //创建透明屏幕禁止屏幕响应
                    g_rotate_ctl_s.mask = lv_obj_create(equip_setting_page_info.page);
                    ...
                    lv_obj_clear_flag(equip_setting_page_info.page, LV_OBJ_FLAG_GESTURE_BUBBLE);
                    ...
                    //获取当前中间项目索引
                    lv_obj_t *cont_col = lv_obj_get_child(equip_setting_page_info.page, 2);
...
static void async_rotate_cb(void *cmd)
{
    ...
    //边界检查
    if (current_index >= (EQUIP_SETTING - 1))
    {
        current_index = EQUIP_SETTING - 1;
    } 
...
static void delete_mask_page(lv_timer_t *timer)
{
    ...
    lv_obj_add_flag(equip_setting_page_info.page, LV_OBJ_FLAG_GESTURE_BUBBLE);
...
static void setting_iterm_click_event_cb(lv_event_cb_t *e)
{
    ...
    if (LV_EVENT_CLICKED == code)
    {
        ...
        {
            last_tabindex = 0;
            lv_subject_set_int(&equip_setting_subject, PAGE_SWITCH_QRCODE);
        }
        else if (index == 1)
        {
            last_tabindex = 1;
            lv_subject_set_int(&equip_setting_subject, PAGE_SWITCH_ABOUT);
        }
        else if (index == 2)
        {
            last_tabindex = 2;
            lv_subject_set_int(&equip_setting_subject, PAGE_SWITCH_RESET);
        }
        else if (index == 3)
        {
            last_tabindex = 3;
            lv_subject_set_int(&equip_setting_subject, PAGE_SWITCH_CERK);
        }
    }
}
...
static void page_back_event_cb(lv_event_t *e)
{
    lv_subject_set_int(&equip_setting_subject, PAGE_SWITCH_BACK);
}
...
static void lv_switch_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    switch_page->old_page = &equip_setting_page_info;

    switch (page_event)
    {
        case PAGE_SWITCH_QRCODE:
            lv_stack_push(&equip_setting_page_info);
            switch_page->new_page = lv_page_settingQr_info_get();
            break;
        case PAGE_SWITCH_ABOUT:
            lv_stack_push(&equip_setting_page_info);
            switch_page->new_page = lv_page_about_camera_info_get();
            break;
        case PAGE_SWITCH_RESET:
            lv_stack_push(&equip_setting_page_info);
            switch_page->new_page = lv_page_factory_restore_info_get();
            break;
        case PAGE_SWITCH_CERK:
            lv_stack_push(&equip_setting_page_info);
